...
class EE_PromptedVisionTransformer(EEVisionTransformer):
    ...
    def forward_features(self, x):
        ...
        if self._m_config.MODEL.E2VPT.KV_PROMPT.VIT_POOL_TYPE == "imgprompt_pool":
            assert self._m_config.MODEL.E2VPT.KV_PROMPT.LOCATION == "prepend"
            x = x[:, 1:, :].mean(dim=1)  # global pool without cls token
            outcome = self.fc_norm(x)
        elif self._m_config.MODEL.E2VPT.KV_PROMPT.VIT_POOL_TYPE == "original":
            x = x[:, 1:, :].mean(dim=1)  # global pool without cls token
            outcome = self.fc_norm(x)
        elif self._m_config.MODEL.E2VPT.KV_PROMPT.VIT_POOL_TYPE == "img_pool":
            assert self._m_config.MODEL.E2VPT.KV_PROMPT.LOCATION == "prepend"
            x = x[:, self.num_tokens_P+1:, :].mean(dim=1)
            outcome = self.fc_norm(x)
        elif self._m_config.MODEL.E2VPT.KV_PROMPT.VIT_POOL_TYPE == "prompt_pool":
            assert self._m_config.MODEL.E2VPT.KV_PROMPT.LOCATION == "prepend"
